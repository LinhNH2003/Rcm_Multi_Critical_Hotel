"""
Hotel recommendation system with semantic similarity and clustering capabilities.
"""
...
try:
    ...
    print("Warning: Some modules not found. Make sure clustering_facilities and config are available.")
...
class AmenityConverter:
    ...
    def convert_amenity(cls, value: Union[int, str]) -> Union[str, int]:
        ...
        if isinstance(value, int):
            return cls.AMENITIES.get(value, "Không tìm thấy")
        elif isinstance(value, str):
            for amenity_id, amenity_name in cls.AMENITIES.items():
...
class ClusterManager:
    ...
    def get_main_cluster(self, clusters: List[Tuple], threshold: float = 0.1) -> List:
        ...
        Args:
            clusters: List of (cluster_id, score) tuples
            threshold: Threshold for including secondary clusters
            
        Returns:
            List of main cluster IDs
        """
        if not clusters:
            return []
    ...
    def suggest_related_facilities(self, user_input: str) -> Tuple[List, List[str]]:
        ...
        Args:
            user_input: User input facility
            
        Returns:
            Tuple of (cluster, suggested_facilities)
        """
        try:
...
class ScoreCalculator:
    ...
    def count_number_occurrences(data: Dict, selected_numbers: List) -> Tuple[Dict, int]:
        ...
        Args:
            ...
            selected_numbers: List of numbers to count
            
        Returns:
            Tuple of (selected_counts, total_count)
        """
    ...
    def get_score_designated_services(self, facilities: Optional[List] = None,
        ...
        Args:
            facilities: List of facilities to score
            selected_numbers: List of numbers to count occurrences for
            facility_type: Type of facilities ("popular" or other)
            
        Returns:
            List of scored facilities
        """
    ...
    def extract_scores(result: List[Dict], use_tuple: bool = False) -> Tuple[List, List]:
        ...
        Args:
            ...
            use_tuple: Whether to use tuple's second value for score
    ...
    def normalize_scores(scores: List[float], beta: float = 1.0, 
        ...
        Args:
            ...
            beta: Beta parameter for adjustment
            use_median: Use median instead of mean for x0
    ...
    def calculate_score(self, result1: List[Dict], result2: List[Dict],
        ...
        Args:
            ...
            beta: Beta parameter for normalization
            use_median: Use median for normalization
            
        Returns:
            List of final scored results
        """
...
class HotelScoreService:
    ...
    def get_score_services(self, user_input: Union[List[int], List[str]],
        ...
        Args:
            user_input: List of amenities (int or str)
            list_ids: Optional list of IDs to filter facilities
            weights: Weights for combining scores
            beta: Beta parameter for normalization
            use_median: Use median for normalization
    ...
    def calculate_hotel_scores(result_hotel: List[Dict], result_room: List[Dict],
        ...
        Args:
            result_hotel: Hotel similarity results
            result_room: Room similarity results
            threshold: Threshold for hotels without rooms
            weight: Weights for combining hotel and room scores
            
        Returns:
            Dictionary of hotel IDs and final scores
        """
        hotel_scores = {}
...
class HotelSimilarityRecommender:
    ...
    def __init__(self, model_name: str = 'paraphrase-multilingual-MiniLM-L12-v2',
                 use_gpu: bool = True, model_dir: str = "/content",
        ...
        Args:
            ...
            model_dir: Directory for storing vectors and indices
            recommendation_type: Type of recommendation ('hotel' or 'room')
            batch_size: Batch size for encoding
            embedding_dim: Embedding dimension (auto-detected if None)
            faiss_metric: FAISS metric ('IP' or 'L2')
    ...
    def embed_amenities(self, amenities: List[str],
        ...
        Args:
            amenities: List of amenities to encode
            input_amenities: Input amenities for similarity calculation
            weights: Custom weights for embeddings
    ...
    def batch_embed_amenities(self, all_amenities: List[List[str]]) -> List[np.ndarray]:
        ...
        Args:
            all_amenities: List of amenity lists for each hotel
            
        Returns:
            List of embedding vectors for each hotel
        """
        flat_amenities = []
        hotel_indices = []
    ...
    def fit(self, hotels: List[Dict[str, Any]], recompute_vectors: bool = False):
        ...
        Args:
            ...
            recompute_vectors: Whether to recompute vectors instead of loading
        """
    ...
    def predict_top_k(self, input_amenities: List[str], top_k: int = 10,
                     ...
                     normalization_factor_base: float = 10.0) -> List[Dict[str, Any]]:
        """
        Predict top K similar hotels/rooms based on input amenities.
        
        Args:
            input_amenities: List of input amenities
            top_k: Number of results to return
            similarity_threshold: Cosine threshold for matched amenities
            normalization_factor_base: Base for normalization factor adjustment
            
        Returns:
            List of prediction results with similarity information
        """
        ...
        for sim, idx in zip(similarities[0], indices[0]):
            ...
            if self.recommendation_type == 'hotel':
    ...
    def predict_assign_id(self, input_amenities: List[str], hotel_ids: List[str],
                         similarity_threshold: float = 0.90,
                         normalization_factor_base: float = 10.0) -> List[Dict[str, Any]]:
        """
        Predict similarity for specific hotel/room IDs based on input amenities.
        
        Args:
            input_amenities: List of input amenities
            hotel_ids: List of specific IDs to predict for
            similarity_threshold: Cosine threshold for matched amenities
            normalization_factor_base: Base for normalization factor adjustment
            
        Returns:
            List of prediction results for specified IDs
        """
        input_vector = self.embed_amenities(
            input_amenities, input_amenities
        ).astype('float32').reshape(1, -1)
        ...
        results = []
        id_field = 'id' if self.recommendation_type == 'hotel' else 'id_room'
        
        # Create mapping from ID to index
        id_to_idx = {
            hotel.get(id_field): idx 
            for idx, hotel in enumerate(self.hotel_data)
        }
        
        for hotel_id in hotel_ids:
            if hotel_id not in id_to_idx:
                # Return empty result for missing IDs
                result = {
    ...
    def reinitialize_model(self, model_name: str):
        """
        Reinitialize with a different Sentence-BERT model.
        
        Args:
            model_name: New Sentence-BERT model name
        """
        self.model_name = model_name
